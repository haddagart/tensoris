...
if [[ "$IS_PRERELEASE" == true ]]; then
    ...
    uv run mike deploy --push --ignore-remote-status --update-aliases dev || true
    ...
    git fetch origin gh-pages:gh-pages 2>/dev/null || true
    uv run mike deploy --push --ignore-remote-status --update-aliases "${RAW_VER}" latest || true
    uv run mike set-default --push latest || true
fi
